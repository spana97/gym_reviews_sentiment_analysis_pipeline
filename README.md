...
Initially, the project will implement an ETL (Extract, Transform, Load) process to combine, clean, and structure both datasets before applying topic modelling techniques.

## Getting Started

### Dependencies

At this stage of the project, the full dependency list is still evolving and will be finalised once the ETL and analysis components are implemented.
...
Installation instructions will be provided once the project structure is established and dependencies are confirmed.

### Executing program

Execution steps will be added after the initial ETL pipeline is implemented. Expected workflow:

1. Run the ETL pipeline to combine and clean Google + Trustpilot reviews.
1. Load the transformed output CSV.
1. Run the topic modelling script (BERTopic) on negative reviews.

Sample command blocks will be included once the scripts exist.
